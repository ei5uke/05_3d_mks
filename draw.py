...
def generate_sphere( points, cx, cy, cz, r, step ):
    bob = new_matrix()
    for rot in range(0, 100): #removes any discrepencies from floating point, wait does it?
        for cir in range(0, 100):
            x = r * math.cos(math.pi * cir/100) + cx
            y = r * math.sin(math.pi * cir/100) * math.cos(2*math.pi * rot/100) + cy
            z = r * math.sin(math.pi * cir/100) * math.sin(2*math.pi * rot/100) + cz
            add_edge(bob, x, y, z, x+1, y+1, z+1)
            cir += step
        rot += step
        cir = 0
    return bob
...
def add_sphere( points, cx, cy, cz, r, step ):
    bob = generate_sphere( points, cx, cy, cz, r, step )
    for sponge in bob:
        points.append(sponge)
...
def generate_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    for rot in range(0, 100): #removes any discrepencies from floating point, wait does it?
        for cir in range(0, 100):
            x = math.cos(2 * math.pi * rot/100) * (r0 * math.cos(2 * math.pi * cir/100) + r1) + cx
            y = r0 * math.sin(2 * math.pi * cir/100) + cy
            z = -math.sin(2 * math.pi * rot/100) * (r0 * math.cos(2 * math.pi * cir/100) + r1) + cz
            add_edge(bob, x, y, z, x+1, y+1, z+1)
            cir += step
        rot += step
        cir = 0
...
def add_torus( points, cx, cy, cz, r0, r1, step ):
    bob = generate_torus( points, cx, cy, cz, r0, r1, step )
    for sponge in bob:
        points.append(sponge)
